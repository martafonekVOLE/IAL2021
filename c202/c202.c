...
void Stack_Init( Stack *stack ) {
	...
	else{						//Pokud byl vytvoren zásobnik (stack), tak ho zaindexuj tak, aby byl prázdný
		stack->topIndex = -1;
	}
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	else{											//Pokud zasobnik (stack) neni prazdny, uloz data na vrcholu zasobniku na adresu
...
void Stack_Pop( Stack *stack ) {
	...
	else{										//Pokud neni zasobnik (stack) prazdny, tak odstran prvek z vrcholu zasobniku
...
void Stack_Push( Stack *stack, char data ) {
	...
	else{										//Pokud neni zasobnik (stack) plny, tak vloz na vrchol zasobniku
		stack->topIndex = stack->topIndex+1;
